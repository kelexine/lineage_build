...
prep_build() {
    ...
    cp ./lineage_build/local_manifests_${MODE}/*.xml .repo/local_manifests
    echo ""
...
apply_patches() {
    ...
    bash ./lineage_build/apply_patches.sh ./lineage_patches/${1}
}
...
build_treble() {
    ...
    make installclean
    make -j$(lscpu -b -p=Core,Socket | grep -v '^#' | sort -u | wc -l) bacon
